...
`@script`
Welcome to the second Chapter of the course. Previously, we learned what is testing and why it is important for keeping our code clean. We also mentioned that it would be great to have a testing framework that could help us handle tests in an easy way. Fortunately, there is already one built-in in the Python standard library.
...
`@script`
If you want, however, to run all the tests contained within your current directory, use the following command. However, for a test module to be found by this command, its name should contain the prefix TEST.
If you want to run tests from a specific directory, you can use a slightly different command.
...
`@script`
Congratuliations! You covered the basics of using unittest module to create and run simple tests. Now it's time to practice.
